...
function toMs(time) {
  ...
  if (length < 13) {
    time = time * Math.pow(10, (13 - length));
  }
  return time;
}
...
InfluxdbBackend.prototype.httpPOST_v09 = function (points) {
  ...
  var self = this,
      query = {u: self.user, p: self.pass, db: self.database, precision: 'ms'},
      protocolName = self.protocol == http ? 'HTTP' : 'HTTPS',
      startTime;
  ...
  var points_to_payload = function(points) {
    ...
    for (p in points) {
      p = points[p];
      var timestamp = p['fields']['timestamp'] != undefined ? toMs(p['fields']['timestamp']) : false;
