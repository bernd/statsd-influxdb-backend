...
function InfluxdbBackend(startupTime, config, events) {
  ...
  self.statsPrefix = config.prefixStats !== undefined ? config.prefixStats : 'statsd';
  ...
  if (config.influxdb) {
    ...
    // SSL Configuration
    if (config.influxdb.ssl && config.influxdb.ssl.enable ) {
      // Make sure we set the protocol to https
      self.protocol = https

      if (config.influxdb.ssl.ca != null) {
        self.ssl.ca = config.influxdb.ssl.ca;
      }

      if (config.influxdb.ssl.cert != null) {
        self.ssl.cert = fs.readFileSync(config.influxdb.ssl.cert);
      }

      if (config.influxdb.ssl.ciphers != null) {
        self.ssl.ciphers = config.influxdb.ssl.ciphers;
      }

      if (config.influxdb.ssl.key != null) {
        self.ssl.key = fs.readFileSync(config.influxdb.ssl.key);
      }

      if (config.influxdb.ssl.passphrase != null) {
        self.ssl.passphrase = config.influxdb.ssl.passphrase;
      }

      if (config.influxdb.ssl.pfx != null) {
        self.ssl.pfx = fs.readFileSync(config.influxdb.ssl.pfx);
      }

      if (config.influxdb.ssl.rejectUnauthorized != null) {
        self.ssl.rejectUnauthorized = config.influxdb.ssl.rejectUnauthorized;
      }

      if (config.influxdb.ssl.secureProtocol != null) {
        self.ssl.secureProtocol = config.influxdb.ssl.secureProtocol;
      }
    }
  }

  // Build the bulk of the http options
  self.httpOptions = {
    hostname: self.host,
    port: self.port,
    method: 'POST',
  ...
  // Merge in the SSL configuration pieces
  self.httpOptions = self.merge(self.httpOptions, self.ssl);
...
InfluxdbBackend.prototype.processFlush = function (timestamp, metrics) {
  var self = this,
      ...
      key, timerKey,
      statsPrefixRegexp = new RegExp('^' + self.statsPrefix + '\\.');
...
InfluxdbBackend.prototype.httpPOST = function (points) {
  ...
  // Clone the options
  var options = clone(self.httpOptions, true);

  // Update the path
  options.path = '/db/' + self.database + '/series?' + querystring.stringify(query);
...
// From: http://jsfiddle.net/amTEH/
InfluxdbBackend.prototype.merge = function() {
  var destination = {},
