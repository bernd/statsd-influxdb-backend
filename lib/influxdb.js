 ...
 *   version: 0.8,        // InfluxDB version. (default 0.8) (options 0.8,0.9)
...
function InfluxdbBackend(startupTime, config, events) {
	...
	self.prefixStats = config.prefixStats !== undefined ? config.prefixStats :
		'statsd';
	...
	if (self.proxyEnable) {
		self.log('Starting the buffer flush interval. (every ' + self.proxyFlushInterval +
			'ms)');
...
InfluxdbBackend.prototype.processFlush = function(timestamp, metrics) {
	...
	for (key in counters) {
		...
		if (value) {
			points.push(self.assembleEvent(k, [{
				value: value,
				time: timestamp
			}]));
		}
	...
	for (set in sets) {
		sets[set].map(function(v) {
			points.push(self.assembleEvent(set, [{
				value: v,
				time: timestamp
			}]));
		})
		points.push(self.assembleEvent(set + "_count", [{
			value: sets[set].length,
			time: timestamp
		}]));
	}
...
InfluxdbBackend.prototype.httpPOST_v08 = function(points) {
	...
	self.logDebug(function() {
		return 'Sending ' + points.length + ' different points via ' +
			protocolName;
	});
...
function csvKeyVal(obj) {
	var csv = [];
	for (var key in obj) {
		if (obj.hasOwnProperty(key)) {
			csv.push(key + "=" + obj[key]);
		}
	}
...
InfluxdbBackend.prototype.makeLineBasedPayload_v09 = function(points) {
	var fmtPayload = [];
	for (var idx in points) {
		var point = points[idx];
		var measurementName = point.name;
		var timestamp = point.timestamp;
		var fields = point.fields;

		var _fmtPayload = measurementName + " " + csvKeyVal(fields) + " " +
			timestamp;
		fmtPayload.push(_fmtPayload);
	}
	return fmtPayload.join("\n");
}
...
InfluxdbBackend.prototype.httpPOST_v09 = function(points) {
	...
	self.logDebug(JSON.stringify(points))
	...
	self.logDebug("url:" + options.hostname +
	...
	// convert payload to new 0.9 format
	var payload = self.makeLineBasedPayload_v09(points);
